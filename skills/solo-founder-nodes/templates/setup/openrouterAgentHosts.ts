...
export function makeOpenRouterAgentSetupPack(input: { generatedAt?: string; hostRoot?: string } = {}): OpenRouterAgentSetupPack {
  ...
  return {
    ...
    verificationCommands: [
      ...
      "openclaw agent --local --agent main --session-key agent:main:solo-founder-conformance --model openrouter/$env:SOLO_OPENROUTER_OPENCLAW_MODEL --message \"Run node skills/solo-founder-nodes/conformance/conformance.mjs --run-smoke and return the PASS/FAIL receipt.\" --timeout 900 --json",
      "hermes --provider openrouter --model $env:SOLO_OPENROUTER_HERMES_MODEL --yolo --oneshot \"Run node skills/solo-founder-nodes/conformance/conformance.mjs --run-smoke and return the PASS/FAIL receipt.\"",
    ],
    safetyRules: [
