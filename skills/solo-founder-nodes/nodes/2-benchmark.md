...
- The gstack operating-lane receipt from `npm run sfn -- gstack recommend --phase benchmark --goal "<goal>"`, so the rubric gets product and engineering review before it hardens.
...
0b. **Assert gstack benchmark review ready.** Run `npm run sfn -- gstack recommend --phase benchmark --goal "<goal>" --perf`. The plan must include product review plus a benchmark/eval lane; if it flags architecture risk, carry the `plan-eng-review` test-matrix receipt into `benchmark-choice.md`.
...
The gstack benchmark receipt is not a score; it is a second check that the benchmark choice and rubric would survive product, engineering, and eval review.
