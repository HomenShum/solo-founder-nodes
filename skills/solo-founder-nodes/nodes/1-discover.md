...
0b. **Intake fresh founder tweaks.** If the user supplied new screenshots, pasted chat, competitor links, roadmap bullets, provider/storage preferences, or "also..." comments, run `npm run sfn -- tweak intake --goal "<goal>" --input <text-or-file> --domain <domain> --out idea-tweaks.json` and `npm run sfn -- tweak verify --receipt idea-tweaks.json`. Use the receipt to update the capability spec, Intent RALPH, Component RALPH, setup, design flow, and proof plan only where the deltas require it.
