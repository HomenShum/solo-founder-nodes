...
The current 2026-06-24 audit selects `deepseek/deepseek-v4-flash` as the cheap paid OpenClaw lane,
`qwen/qwen3-coder-next` as the Hermes/coding fallback, `cohere/north-mini-code:free` as an optional
free coding lane, and `google/gemini-3.1-flash-lite` as the optional multimodal UI lane. Treat those
as receipt-backed defaults, not permanent truth: rerun the OpenRouter smoke and agent conformance
before publishing model claims.
